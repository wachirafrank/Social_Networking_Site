...
if (isset($_POST['text']))
{
$text = sanitizeString($link,$_POST['text']);
if ($text != "")
{
$pm = substr(sanitizeString($link,$_POST['pm']),0,1);
$time = time();
queryMysql($link,"INSERT INTO messages VALUES(NULL, '$user',
'$view', '$pm', $time, '$text')");
}
}
if ($view != "")
...
if (isset($_GET['erase']))
{
$erase = sanitizeString($link,$_GET['erase']);
queryMysql($link,"DELETE FROM messages WHERE id=$erase AND recip='$user'");
}
$query = "SELECT * FROM messages WHERE recip='$view' ORDER BY time DESC";
$result = queryMysql($link,$query);
